            ...
            <div className="bg-white">
                <div className="mx-auto max-w-2xl px-4 sm:py-12 sm:px-6 lg:max-w-7xl lg:px-8">
                    <div className='py-2 pb-5'>
                        <h2 className="text-2xl font-bold tracking-tight text-gray-700">RESULTS - {query.toUpperCase()}</h2>
...
export async function getServerSideProps(context) {
    ...
    let products = {}
    const gender = ['men', 'Men', 'MEN', 'women', 'Women', 'WOMEN']
    const categories = ['shirts','shirt', 'tshirts', 'tshirt', 'hoodie', 'hoodies', 'pants', 'sweatshirt','jeans']
    const mencategories = ['menshirts', 'menshirt', 'mentshirts', 'mentshirt', 'menhoodie', 'menhoodies', 'menpants', 'mensweatshirt','menjeans']
    const womencategories = ['womenshirts', 'womenshirt', 'womentshirts', 'womentshirt', 'womenhoodie', 'womenhoodies', 'womenpants', 'womensweatshirt', 'womenjeans']


    if (gender.includes(query)) {
        products = await Product.find({ 'gender': query })
    }
    if (categories.includes(query)) {
        products = await Product.find({ 'category': query })
    }
    if (womencategories.includes(query)) {
        const cat = query.substring(5, query.length)
        products = await Product.find({ 'gender': 'women' , 'category': cat})
    }
    if (mencategories.includes(query)) {
        const cat = query.substring(3, query.length)
        products = await Product.find({ 'gender': 'men','category': cat })
    }
